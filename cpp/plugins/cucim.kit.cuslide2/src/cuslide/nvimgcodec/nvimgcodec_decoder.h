...
/**
 * Convenience overload that takes a reference to pointer.
 * Simply delegates to the pointer-to-pointer version above.
 */
inline bool decode_ifd_region_nvimgcodec(const IfdInfo& ifd_info,
                                         nvimgcodecCodeStream_t main_code_stream,
                                         uint32_t x, uint32_t y,
                                         uint32_t width, uint32_t height,
                                         uint8_t*& output_buffer,
                                         const cucim::io::Device& out_device)
{
    return decode_ifd_region_nvimgcodec(ifd_info, main_code_stream,
                                        x, y, width, height,
                                        &output_buffer, out_device);
}
